...
#### 施策1: APIプロンプトのmood指示強化 (gui.py)
- `"mood": "雰囲気"` → `"mood": "シーンの空気感・雰囲気（15-30字の情景描写。1-2語のラベル禁止）"`
- バッチ生成(L9638)・単体生成(L11627)の2箇所を変更
- APIが「興奮」「緊張」等の1-2語ラベルを返す問題を抑制

#### 施策2: emotional_arc連続性チェック＋自動修正 (gui.py)
- validate_script: 各シーンのemotional_arc.endと次シーンのstartを比較し不一致を警告。30%超でglobal警告
- auto_fix Step 10f新設: 前シーンのemotional_arc.endを次シーンのstartにコピーして感情断絶を修正
- チャンク境界での感情接続断絶を検出・自動修復

#### 施策3:「堕ち」パターン免除テーマ拡張 (gui.py)
- Step 10b感情矛盾免除キーワード: 4語 → 18語に拡張
- 追加: corruption/堕ち/調教/hypnosis/催眠/洗脳/chikan/痴漢/humiliation/凌辱/屈辱/netorare/ntr/寝取/incest/近親
- 「堕ち」が正常パターンであるテーマで誤修正されていた問題を解消
...
#### THOUGHT_POOL文語的・描写的エントリー根絶 (ero_dialogue_pool.py)
- body_awareness: `脳みそ…とけて…` → `とけてく…`（部位ラベル冒頭除去）
- vanilla: `胸がいっぱい`/`心もとろける`/`心が満たされ`/`心まで触れて`/`ドキドキ…つたわる` → 話し言葉の断片に置換
- ntr: `体が覚えてる` → `覚えちゃってる…`
- corruption: `体だけじゃない`/`脳が溶けて…`/`思考が消える` → 感覚主体の断片に置換
- 計10件のエントリーを現代口語の感覚型表現に修正

#### auto_fix Step 9d新設: 描写型thought自動検出 (gui.py)
- `体が`/`脳が`/`心が`/`胸が`/`思考が`/`意識が`/`理性が`/`本能が`/`感覚が`/`心臓が`/`神経が`/`頭が` の12パターンを正規表現で検出
- 既存の部位ラベル冒頭検出(Step 9b)では捕捉できない「主語＋述語」型ナレーションthoughtを自動置換

#### auto_fix Step 9e新設: 男性セリフ誤割り当て検出 (gui.py)
- 女性専用表現（♡/やだ/だめぇ/きもちぃ/んほぉ/あへぇ/イっちゃ等）が男性speakerに割り当てられている場合を自動検出
- テーマ・intensityに応じた適切な男性セリフプールから代替を選んで置換

#### 男性セリフプール intensity分離 (gui.py)
- `_get_male_pool_for_theme()` のelse分岐（テーマ未指定時）を3段階に分離:
  - intensity ≤ 2: `gentle` + `foreplay` + `praise`（告白・日常シーンに適切な穏やかな表現）
  - intensity 3: `dirty` + `praise`（従来通り）
  - intensity ≥ 4: `command` + `dirty`（従来通り）
- auto_fix用の重複関数（auto_fix_script内）も同様に修正

#### 男性観察型「～のか」パターン追加 (gui.py)
- `_MALE_OBSERVATION_PATTERNS` に10パターン追加: `感じてるのか`→`感じろ` / `欲しいのか`→`欲しいんだろ` / `イきたいのか`→`イケ` 等
- `_MALE_OBS_RE` 正規表現に `のか|んだな` を追加（従来の `だな|するな|してるな|だろうな|てるな` に加えて）

#### MOAN_POOL濁点母音エントリー置換 (ero_dialogue_pool.py) ※前セッション実施
- intensity 4-5の `あ゛`/`お゛`/`ん゛`/`い゛`/`う゛` 系175エントリーを自然な延音・破裂音に置換
- `あ゛` → `あぁ` / `お゛` → `おぉ` / `ん゛` → `んん` 等

#### thought few-shot注入 (gui.py) ※前セッション実施
- generate_scene_draftのキャラプールセクションにthought例を追加注入
- `※thoughtは感覚・感情主体で書け。部位ラベル冒頭禁止` 指示を併記

#### auto_fix Step 9b正規表現拡張 (gui.py) ※前セッション実施
- `_BODY_PART_LABELS` に `頭`/`からだ`/`体`/`あたま`/`心臓`/`脳みそ`/`指先`/`つま先` の8語追加
- `_BODY_PART_RE` パターンを `[がのはを]?…` に拡張（助詞付きパターンも捕捉）

#### THOUGHT_POOL general部位主語エントリー置換 (ero_dialogue_pool.py) ※前セッション実施
- 15件の部位主語型エントリーを感覚主体に修正（`頭…真っ白…`→`真っ白…なにも…` 等）

#### システムプロンプト強化 (gui.py)
- thoughtの文語的表現禁止を明示（❌`胸がときめく`/`心が満たされ`/`魂が震え`）
- 描写型（`体が～`/`脳が～`/`心が～`）と感覚型（`熱い…`/`ゾクって…`/`とけちゃう…`）の対比例を追加

#### cg_bubble_writer.skill.md修正 ※前セッション実施
- intensity 5の喘ぎ例 `あ゛…っ♡♡` → `あぁぁっ♡♡` に修正
