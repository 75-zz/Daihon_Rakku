"""エロセリフプール v9.8: 重複置換用の大量セリフ辞書（CG集吹き出し用）

合計4488エントリー + STORY_PATTERN_LIBRARY(598パターン):
  MOAN_POOL: 5 intensity × 160 = 795（v8.0: 80→160に倍増、500シーン耐性）
  SPEECH_FEMALE_POOL: 59 categories = 1576
    ...
    v9.7: +haramase 30 +gal_speech 30 +oneesan 25 +hitozuma 25 +jirai 25
          +harem_jealous 20 +streaming 20 +pregnancy_aware 20
    v9.8: +fuzoku_react 25 +exposure_react 25 +wakarase_react 25 +josoukai_react 20
          +tanetsuke_react 20 +virgin_react 20 +cohabitation 20
  THOUGHT_POOL: 41 categories = 937
    ...
    v8.0: +anal_thought 15 +toy_thought 15 +oral_thought 15 +orgasm_thought 20
          +sensitivity 15
    v9.7: +haramase_thought 20 +gal_thought 15 +oneesan_thought 15
          +hitozuma_thought 15 +jirai_thought 15
    v9.8: +fuzoku_thought 15 +exposure_thought 15 +wakarase_thought 15 +virgin_thought 15
  SPEECH_MALE_POOL: 34 categories = 846
    ...
    v9.7: +haramase_male 20 +oneesan_tease 15 +hitozuma_dirty 15 +streamer_male 15
    v9.8: +fuzoku_client 15 +wakarase 15 +josoukai 15 +exposure_command 15
          +tanetsuke 15 +shojo_male 15 +dosei_male 15
  ONOMATOPOEIA_POOL: 25 categories = 135（v9.8: +tanetsuke_press/massage/clothing_rustle）
  NEUTRAL_POOL: 2 categories = 125 (非エロシーン用、v9.8拡充)
  AFTERMATH_POOL: 2 categories = 74 (事後シーン用)
  MALE_SHORT_REPLACEMENTS: 男性セリフ短縮辞書 = 50+
  STORY_PATTERN_LIBRARY: 598パターン（A-Gグループ + v6.0拡張）
    v9.7: +story_patterns_ext_f.py (50パターン: 孕ませ/ギャル/お姉さん/ハーレム/地雷/配信者/人妻/催眠/シチュエーション)
    v9.8: +story_patterns_ext_g.py (50パターン: 風俗/常識改変/わからせ/露出/快楽堕ち/種付けプレス/処女/JK/同棲/部活)
"""
import random
...
ONOMATOPOEIA_POOL = {
    ...
    "kiss": {
        "light": ["ちゅっ", "ちゅ…", "んちゅ…", "ぺちゅ", "ちゅぅ…", "ちゅん…", "ふちゅ…", "ん…ちゅ", "こつん…ちゅ", "ちゅ…ん", "ぷちゅ…", "はむ…ちゅ"],
        "deep":  ["ちゅぷ…", "じゅるっ", "んちゅるる", "れろ…ちゅ…", "ちゅぷちゅぷ", "じゅるじゅる", "れろんちゅぷ", "んちゅぅ…", "ちゅるるる", "じゅぶっ", "れろれろ…ちゅぷ", "んぐ…ちゅるっ"],
        "wet":   ["じゅる…ちゅ…", "ちゅるん…", "れろ…じゅるっ", "んちゅ…じゅるる", "ちゅぱ…じゅるっ", "じゅぶ…ちゅるる", "れろんれろん", "んちゅぷ…じゅる"],
    },
    "anal": {
    ...
    "footjob": {
        ...
        "fast":   ["するするする", "すりすりすりっ", "にゅるにゅるにゅる", "ぐにぐにぐに", "くちゅくちゅくちゅ", "ぬるぬるぬるっ", "もにゅもにゅもにゅ", "するっするっする"],
    },
    "paizuri": {
        "gentle": ["むにゅ…", "ぷにゅ…", "ふにゅ…", "むにむに…", "たぷ…ん", "もにゅ…", "ぷにっ…", "ふわ…っ"],
        "normal": ["むにゅむにゅ", "ぱふぱふ", "ぬるぬる", "たぷんたぷん", "むにゅんむにゅん", "もみもみ", "ぷにぷに", "ぬちゅぬちゅ"],
        "fast":   ["ぱふぱふぱふ", "むにゅんむにゅんむにゅん", "ぬるぬるぬる", "たぷたぷたぷ", "もみもみもみ", "ぷにぷにぷに", "ぬちゅぬちゅぬちゅ", "むにゅむにゅむにゅ"],
    },
    "finger": {
    ...
    # 種付けプレス音
    "tanetsuke_press": {
        "impact":  ["ズンッ", "ズン…ズン…", "ズンズンッ", "ドスンッ", "ドンッ", "ズドンッ", "ドスッ…ドスッ…", "ズシンッ"],
        "release": ["ドクドクッ", "ドクン…ドクン…", "ドクッ…ドクッ…", "ビュルルッ", "ドプドプ", "ビュクッ", "ドクドクドク", "ビュルッ"],
        "grip":    ["ギュウッ", "ギュッ…ギュッ…", "ギリギリッ", "ムギュッ", "ぎゅうぅ", "がしっ", "ぐっ…ぐっ…", "きゅっ…"],
    },
    # マッサージ・エステ音
    "massage": {
        "stroke": ["スリスリ", "スルスル", "サワサワ", "ツツーッ", "なでなで", "すぅっ…", "さらさら", "つぅーっ"],
        "knead":  ["モミモミ", "モミッ", "グニグニ", "ムニュッ", "もにゅ…", "ぐにっ", "むにむに", "もみもみ"],
        "oil":    ["ヌルヌル", "ヌルッ", "トロトロ", "ニュルッ", "ぬめぬめ", "とろ…ん", "ぬるり", "にゅるにゅる"],
    },
    # 衣擦れ音
    "clothing_rustle": {
        "remove": ["シュルッ", "スルッ", "サラッ", "シャッ", "するする", "さらっ…", "しゅるる", "すとん"],
        "drop":   ["パサッ", "バサッ", "ヒラッ", "フワッ", "ぽとっ", "はらり", "ふわっ…", "ぱさり"],
        "tear":   ["ビリッ", "ビリビリッ", "バリッ", "メリッ", "びりっ…", "ばりばり", "べりっ", "ぶちっ"],
    },
}
