"""エロセリフプール v7.0: 重複置換用の大量セリフ辞書（CG集吹き出し用）

合計4400+エントリー + STORY_PATTERN_LIBRARY(46パターン/690 key_lines):
  MOAN_POOL: 5 intensity × 80 = 400（intensity4を80に補完）
  SPEECH_FEMALE_POOL: 36 categories = 990
  ...
  THOUGHT_POOL: 27 categories = 701
  ...
  SPEECH_MALE_POOL: 20 categories = 480
    v6.0: +fantasy_hero/ojisan/doctor/delinquent
    v7.0: +teacher 20 +yakuza 20 +classmate 20 +stepfather 20
  ONOMATOPOEIA_POOL: 17 categories = 185+（+anal/throat/vibrator/dripping/breathing）
...
AFTERMATH_POOL = {
    ...
    "male": [
        ...
        "いい顔してんな", "もう立てねぇか", "壊れちまったか",
        "掃除しとけ", "今日は許してやる", "お前…いい女だな",
        "癖になりそうだ",
    ],
}
...
ONOMATOPOEIA_POOL = {
    ...
    "spanking": {
        "light":  ["ぱしっ", "ぺちん", "ぱちっ", "ぱん"],
        "normal": ["ぱぁん", "ばちん", "ぱしぃん", "ばちっ"],
        "hard":   ["ばちぃん", "ぱぁぁん"],
    },
    "suppressed": ["んんっ…", "ぅ…っ", "んぐっ…", "くっ…ん", "ん…ん…", "ぅぅ…っ", "んむっ…", "んっ…んっ…"],
    "crying": ["ひっく…", "ぐすっ…", "えぐっ…", "うっ…ぅ…", "しくしく", "ぐすん…", "ひっく…ひっく…"],
    # v7.0追加カテゴリ
    "anal": {
        "gentle": ["ぬるっ…", "むにゅ…", "つぷ…", "きゅむ…"],
        "normal": ["ずぷっ", "きゅぷっ", "ぬぷぷ", "ぐぷっ…", "にゅるん"],
        "hard":   ["ずぼっ", "ぐぱっ", "ぐりゅっ", "ずぷぷっ"],
    },
    "throat": {
        "shallow": ["くぷっ", "ちゅぷ…", "こくん"],
        "deep":    ["んぐぅ…", "ごぼっ", "おえっ…", "ぐぷぅ…", "じゅるるる"],
        "gag":     ["ごほっ", "けほっ", "うぇっ", "んぐぐ…"],
    },
    "vibrator": ["ぶーん", "ぶぃぃ…", "ぴぴぴ", "ぶるるる", "ぶーんぶーん"],
    "dripping": ["ぽたっ", "たらー", "ぽたぽた", "つー…", "とろー"],
    "breathing": {
        "light": ["はぁ…はぁ…", "すぅ…はぁ…", "ふぅ…ふぅ…"],
        "heavy": ["はぁはぁはぁ", "ぜぇぜぇ", "ひゅーひゅー"],
    },
}

# intensity別オノマトペ推奨マップ
ONOMATOPOEIA_INTENSITY_MAP = {
    1: {"categories": ["heartbeat", "kiss", "breathing"], "sub": "light", "count": (0, 1)},
    2: {"categories": ["wet", "kiss", "body", "breathing"], "sub": "light", "count": (1, 1)},
    3: {"categories": ["insertion", "wet", "piston", "oral"], "sub": "normal", "count": (1, 2)},
    4: {"categories": ["piston", "wet", "body", "anal"], "sub": "normal", "count": (2, 3)},
    5: {"categories": ["piston", "ejaculation", "body", "wet", "dripping"], "sub": "heavy", "count": (2, 4)},
}
...
def infer_phase(intensity: int, scene_index: int, total_scenes: int) -> str:
    ...
    if intensity <= 1:
        ...
        return "foreplay" if ratio < 0.5 else "penetration"
    elif intensity == 4:
        return "penetration" if ratio < 0.7 else "climax"
    else:
        return "climax" if ratio < 0.9 else "afterglow"
...
def get_speech_pool(bubble_type: str, theme: str = "", intensity: int = 3,
    ...
    phase指定時はSCENE_PHASE_SPEECH_MAPからも混合。"""
    ...
    # フェーズ別プール混合
    if phase and phase in SCENE_PHASE_SPEECH_MAP:
        phase_pool = SCENE_PHASE_SPEECH_MAP[phase].get(bubble_type, [])
        if phase_pool:
...
MALE_SHORT_REPLACEMENTS = {
    ...
    "膣、こんなに締まってんぞ…最高だ": "極上だな",
    "腰が勝手に動いてんぞ…本当はほしいんだろ": "素直だな",
    "限界だな…もう堪えられてねぇ": "限界だろ",
    "もう限界だな…声漏れてんぞ": "漏れてんぞ",
    "いい顔になってんな": "いい顔だ",
    "もう逃げられねぇ": "もう遅え",
